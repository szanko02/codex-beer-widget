...
if (index >= 0 && !process.argv[index + 1]) throw new Error('Missing directory');
const directory = resolve(index >= 0 ? process.argv[index + 1] : '../.local/relay');
const config = JSON.parse(readFileSync(resolve(directory, 'publisher.json'), 'utf8'));
const ca = process.argv.includes('--public') ? undefined : readFileSync(resolve(directory, 'tls-cert.pem'));
...
console.log('Pairing QR saved to the selected directory as pairing.png; one use, expires in five minutes.');
