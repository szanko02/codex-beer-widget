...
std::wstring codex_path() {
    ...
    if (n && n < 32768 && std::filesystem::is_regular_file(value))
        return value;
    n = GetEnvironmentVariableW(L"APPDATA", value, 32768);
    if (n && n < 32768) {
        auto path =
            std::filesystem::path(value) / L"npm/node_modules/@openai/codex/node_modules/@openai/"
                                           L"codex-win32-x64/vendor/x86_64-pc-windows-msvc/bin/codex.exe";
...
CodexSource::CodexSource(const std::wstring &executable, const std::atomic_bool *caller_stop) {
    ...
    try {
        ...
        check(attributes && InitializeProcThreadAttributeList(attributes, 1, 0, &size),
              "attribute allocation failed");
        HANDLE inherited[] = {child_in, child_out, child_err};
        check(UpdateProcThreadAttribute(attributes, 0, PROC_THREAD_ATTRIBUTE_HANDLE_LIST, inherited,
                                        sizeof(inherited), nullptr, nullptr),
              "handle whitelist failed");
        STARTUPINFOEXW si{};
        ...
        check(SetInformationJobObject(job_, JobObjectExtendedLimitInformation, &limit, sizeof(limit)),
              "job limit failed");
        ...
        request("initialize",
                {{"clientInfo",
                  {{"name", "codex_beer_widget"}, {"title", "Codex Beer Widget"}, {"version", "0.1.0"}}}},
                caller_stop ? *caller_stop : stop);
...
void CodexSource::send(const Json &message) {
    ...
    DWORD written{};
    check(WriteFile(input_, line.data(), static_cast<DWORD>(line.size()), &written, nullptr) &&
              written == line.size(),
          "Codex input closed");
}
std::vector<Json> CodexSource::receive() {
    ...
    for (HANDLE stream : {error_, output_}) {
        ...
        while (available) {
            char bytes[8192];
            DWORD received{};
            check(ReadFile(stream, bytes, (std::min)(available, DWORD(sizeof(bytes))), &received, nullptr) &&
                      received,
                  "Codex pipe closed");
            if (stream == output_) {
    ...
    while ((end = buffered_.find('\n')) != std::string::npos) {
        ...
        if (message.contains("method") && message.contains("id"))
            send({{"id", message["id"]},
                  {"error", {{"code", -32601}, {"message", "Method not supported by quota widget"}}}});
        else
    ...
    while (!stop && std::chrono::steady_clock::now() < deadline) {
        ...
        for (const auto &m : receive()) {
            if (m.contains("id") && m["id"] == id) {
                if (m.contains("error"))
                    throw std::runtime_error("Codex rejected request; check sign-in and connection");
                check(m.contains("result"), "Missing RPC result");
                result = m["result"];
                found = true;
            }
            if (on_notification && m.contains("method"))
                on_notification(m);
        }
        if (found)
            return result;
...
Json CodexSource::resources() const {
    ...
    check(QueryInformationJobObject(job_, JobObjectBasicAccountingInformation, &accounting,
                                    sizeof(accounting), nullptr),
          "Job accounting failed");
    alignas(JOBOBJECT_BASIC_PROCESS_ID_LIST) unsigned char
        storage[sizeof(JOBOBJECT_BASIC_PROCESS_ID_LIST) + 64 * sizeof(ULONG_PTR)]{};
    auto *ids = reinterpret_cast<JOBOBJECT_BASIC_PROCESS_ID_LIST *>(storage);
    check(QueryInformationJobObject(job_, JobObjectBasicProcessIdList, ids, sizeof(storage), nullptr),
          "Process accounting failed");
    double working = 0, committed = 0;
    for (DWORD i = 0; i < ids->NumberOfProcessIdsInList; ++i) {
        HANDLE p = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION | PROCESS_VM_READ, FALSE,
                               static_cast<DWORD>(ids->ProcessIdList[i]));
        if (!p)
            continue;
    ...
    return {{"activeProcesses", accounting.ActiveProcesses},
            {"totalProcesses", accounting.TotalProcesses},
            {"workingSetMiB", working},
            {"privateMiB", committed},
            {"cpuSeconds",
             static_cast<double>(accounting.TotalKernelTime.QuadPart + accounting.TotalUserTime.QuadPart) /
                 10000000}};
}
} // namespace beer
