...
inline std::optional<int64_t> integer_field(const Json& j, const char* key) {
    if (!j.contains(key) || !j[key].is_number_integer()) return {};
    try { return j[key].get<int64_t>(); } catch (...) { return {}; }
}
inline QuotaWindow parse_window(const Json& j, const std::string& slot) {
    QuotaWindow w; w.slot = slot;
    if (!j.is_object()) return w;
    if (j.contains("usedPercent") && j["usedPercent"].is_number()) {
        double used = j["usedPercent"].get<double>();
        if (std::isfinite(used)) w.remaining = std::clamp(100.0 - used, 0.0, 100.0);
    }
    w.minutes = integer_field(j, "windowDurationMins");
    w.resets_at = integer_field(j, "resetsAt");
    if (w.minutes && *w.minutes <= 0) w.minutes.reset();
    if (w.resets_at && *w.resets_at <= 0) w.resets_at.reset();
    return w;
}
inline std::map<std::string, QuotaGroup> parse_groups(const Json& response) {
    ...
    if (!groups.is_object() || groups.empty()) {
        ...
        if (!legacy.is_object()) return result;
        const auto id = legacy.contains("limitId") && legacy["limitId"].is_string() ? legacy["limitId"].get<std::string>() : "legacy";
        groups = {{id, legacy}};
    }
    for (const auto& [id, j] : groups.items()) {
        if (!j.is_object()) continue;
        QuotaGroup group; group.id = id; group.name = id;
        if (j.contains("limitName") && j["limitName"].is_string()) group.name = j["limitName"].get<std::string>();
        for (const auto* slot : {"primary", "secondary"}) {
            if (j.contains(slot) && j[slot].is_object()) group.windows.push_back(parse_window(j[slot], slot));
        }
...
struct QuotaState {
    ...
    void accept(const Json& response, bool partial = false) {
        ...
        if (partial) for (auto& [id, group] : fresh) groups[id] = std::move(group);
        else groups = std::move(fresh);
        updated = std::chrono::system_clock::now(); stale = false; error.clear();
    }
    void fail(std::string message) { stale = true; error = std::move(message); }
    const QuotaGroup* select_group(const std::string& preferred) const {
        if (auto it = groups.find(preferred); it != groups.end()) return &it->second;
        if (!preferred.empty()) return nullptr;
        if (auto it = groups.find("codex"); it != groups.end()) return &it->second;
...
public:
    ...
    void set(double target, double seconds, bool immediate = false) {
        ...
        from_ = immediate ? target : value(now); target_ = std::clamp(target, 0.0, 100.0);
        start_ = now; seconds_ = std::clamp(seconds, .1, 5.0);
    }
    bool active() const { return from_ != target_ && std::chrono::duration<double>(std::chrono::steady_clock::now() - start_).count() < seconds_; }
};
}
