...
void Renderer::resize(UINT width, UINT height) {
    ...
    hr(dc_->CreateBitmapFromDxgiSurface(surface.Get(), &properties, &bitmap_));
...
void Renderer::text(const std::wstring &value, D2D1_RECT_F bounds, uint32_t rgb) {
    color(rgb);
    dc_->DrawTextW(value.c_str(), static_cast<UINT32>(value.size()), text_.Get(), bounds, brush_.Get(),
...
void Renderer::update_theme(const Theme &t) {
    ...
    if (!cached_valid_ || t.text_size != cached_.text_size) {
        text_.Reset();
        hr(write_->CreateTextFormat(L"Segoe UI", nullptr, DWRITE_FONT_WEIGHT_SEMI_BOLD,
...
void Renderer::draw(const Theme &t, double remaining, double secondary, const std::wstring &label,
    ...
    color(t.text_color);
    dc_->DrawTextW(caption.c_str(), static_cast<UINT32>(caption.size()), small_text_.Get(),
                   D2D1::RectF(28, 285, 212, 300), brush_.Get(), D2D1_DRAW_TEXT_OPTIONS_CLIP);
    hr(dc_->EndDraw());
    if (layered)
...
void Renderer::layered_present(HWND window) {
    ...
    if (!staging_) {
        ...
        if (!memory_dc_ || !dib_)
            throw std::runtime_error("Layered window allocation failed");
