...
int main(int argc, char**) {
    try {
        ...
        if (argc > 1) {
            ...
            int notices = 0;
            source.on_notification = [&](const beer::Json& m) { if (m.value("method", "") == "account/rateLimits/updated") notices++; };
            for (int i = 0; i < 4; ++i) {
                std::this_thread::sleep_for(std::chrono::seconds(15));
                source.request("account/rateLimits/read", beer::Json::object(), stop);
            }
            const double elapsed = std::chrono::duration<double>(std::chrono::steady_clock::now() - observed).count();
            auto after = source.resources();
            result["measurement"] = {{"before", before}, {"after", after}, {"elapsedSeconds", elapsed},
                {"notifications", notices}, {"averageCpuOneCorePercent", 100 * (after["cpuSeconds"].get<double>() - before["cpuSeconds"].get<double>()) / elapsed}};
        }
