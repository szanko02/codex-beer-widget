...
int main(int argc, char **) {
    if (argc > 1) {
        ...
        while (std::getline(std::cin, line)) {
            ...
            else if (method == "account/read" && ready)
                result = {{"account", {{"type", "chatgpt"}}}};
            else if (method == "account/rateLimits/read" && ready) {
                std::cout
                    << Json{{"method", "account/rateLimits/updated"}, {"params", {{"test", true}}}}.dump()
                    << std::endl;
                result = {{"rateLimits", {{"primary", {{"usedPercent", 25}}}}}};
            } else {
                std::cout
                    << Json{{"id", m["id"]}, {"error", {{"code", -32601}, {"message", "unknown"}}}}.dump()
                    << std::endl;
                continue;
            }
    ...
    try {
        ...
        if (limits["rateLimits"]["primary"]["usedPercent"] != 25 || notices != 1)
            throw std::runtime_error("message routing failed");
        bool rejected = false;
        try {
            source.request("unknown", Json::object(), stop);
        } catch (const std::runtime_error &) {
            rejected = true;
        }
        if (!rejected)
            throw std::runtime_error("RPC error ignored");
